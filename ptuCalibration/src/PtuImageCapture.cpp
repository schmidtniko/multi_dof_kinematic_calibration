...
void PtuImageCapture::startCapture(const std::function<std::map<int, cv::Mat> (double, double, int&, int&) >& captureFunction,
                                   ...
                                   bool preferTilt)
{
    ...
    if(panStartAngle > panEndAngle)
        std::swap(panStartAngle, panEndAngle);

    if (tiltStartAngle > tiltEndAngle)
        std::swap(tiltStartAngle, tiltEndAngle);
