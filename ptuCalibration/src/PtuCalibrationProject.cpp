...
template<typename T>
inline Eigen::Map<const Eigen::Matrix<T,2,1> > eMap2(const T* v) { return Eigen::Map<const Eigen::Matrix<T,2,1> >(v); }
template<typename T>
inline Eigen::Map<const Eigen::Matrix<T,3,1> > eMap3(const T* v) { return Eigen::Map<const Eigen::Matrix<T,3,1> >(v); }
template<typename T>
inline Eigen::Map<const Eigen::Matrix<T,4,1> > eMap4(const T* v) { return Eigen::Map<const Eigen::Matrix<T,4,1> >(v); }
template<typename T>
inline Eigen::Map<const Eigen::Matrix<T,6,1> > eMap6(const T* v) { return Eigen::Map<const Eigen::Matrix<T,6,1> >(v); }
template<typename T>
inline Eigen::Map<const Eigen::Quaternion<T> > eMapq(const T* v) { return Eigen::Map<const Eigen::Quaternion<T> >(v); }

template<typename T>
inline Eigen::Map<Eigen::Matrix<T,2,1> > eMap2(T* v) { return Eigen::Map<Eigen::Matrix<T,2,1> >(v); }
template<typename T>
inline Eigen::Map<Eigen::Matrix<T,3,1> > eMap3(T* v) { return Eigen::Map<Eigen::Matrix<T,3,1> >(v); }
template<typename T>
inline Eigen::Map<Eigen::Matrix<T,4,1> > eMap4(T* v) { return Eigen::Map<Eigen::Matrix<T,4,1> >(v); }
template<typename T>
inline Eigen::Map<Eigen::Matrix<T,6,1> > eMap6(T* v) { return Eigen::Map<Eigen::Matrix<T,6,1> >(v); }
template<typename T>
inline Eigen::Map<Eigen::Quaternion<T> > eMapq(T* v) { return Eigen::Map<Eigen::Quaternion<T> >(v); }

template<typename T>
void poseAccum(const Eigen::Matrix<T,4,1>& q1, const Eigen::Matrix<T,3,1>& t1, const Eigen::Matrix<T,4,1>& q2, const Eigen::Matrix<T,3,1>& t2, 
	Eigen::Matrix<T,4,1>& q3, Eigen::Matrix<T,3,1>& t3)
{
...
template<typename T>
Eigen::Matrix<T,4,1> cquatConjugate(const Eigen::Matrix<T,4,1>& q)
{
...
template<typename T>
Eigen::Matrix<T,7,1> cposeInv(const Eigen::Matrix<T,7,1>& a)
{
    Eigen::Matrix<T,7,1> ret;
...
template<typename T>
Eigen::Matrix<T,6,1> cposeManifoldMinus(const Eigen::Matrix<T,7,1>& a, const Eigen::Matrix<T,7,1>& b)
{
...
struct OpenCVReprojectionError {
    ...
    template <typename T>
    bool operator()(const T* const camera_xyz,
                    const T* const camera_q,
                    const T* const point_xyz,
                    T* residuals) const {
            ...
            T xp = tagCorC[0];
            T yp = tagCorC[1];
            T xd = xp*(T(1)+r2*(k1 + r2*(k2+ r2*k3))) + T(2)*p1*xp*yp + p2*(r2 + T(2)*xp*xp);
            T yd = yp*(T(1)+r2*(k1 + r2*(k2+ r2*k3))) + T(2)*p2*xp*yp + p1*(r2 + T(2)*yp*yp);
...
// The error is the difference between the predicted and observed position.
            ...
            if (print)
            {
                std::cout << "Predicted: " << predicted_x << " " << predicted_y << " Observed: " << observation.x() << " " << observation.y() << std::endl;
            }
    ...
    static ceres::CostFunction* Create(const Eigen::Vector2d& observation,
            const Eigen::Matrix<double, 5, 1>& d,
            const Eigen::Matrix3d& K) {
        return (new ceres::AutoDiffCostFunction<OpenCVReprojectionError, 2, 3, 4, 3>(          //  residuals, camPose, tagPose
...
struct GaussianPrior1D
{
    ...
    // Factory to hide the construction of the CostFunction object from
    // the client code.
    static ceres::CostFunction* Create(const double mu,
                                       const double sigma)
    {
	...
	// Read Reconstructions
 	{
 		...
 		camSurv::parseReconstructions(folder + "/reconstruction.json",
 			reconstructedTags,
...
//-----------------------------------------------------------------------------
bool PtuCalibrationProject::computeRelativeCameraPoseFromImg(int imageId,
 	const Eigen::Matrix3d& K,
